...
result_t YDlidarDriver::waitPackage(node_info *node, uint32_t timeout) {
  ...
  if (CheckSumResult) {
    ...
    if ((FirstSampleAngle + IntervalSampleAngle * package_Sample_Index) < 0) {
      (*node).angle_q6_checkbit = (((uint16_t)(FirstSampleAngle + IntervalSampleAngle
                                    *
                                    package_Sample_Index + 360 * 64)) << 1) +
                                  LIDAR_RESP_MEASUREMENT_CHECKBIT;
    } else {
      if ((FirstSampleAngle + IntervalSampleAngle * package_Sample_Index) > 360
          * 64) {
        (*node).angle_q6_checkbit = (((uint16_t)(FirstSampleAngle + IntervalSampleAngle
                                      *
                                      package_Sample_Index - 360 * 64)) << 1) +
